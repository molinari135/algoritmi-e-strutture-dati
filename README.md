...
Ogni albero n-ario ordinato T si può rappresentare con un albero binario B avente gli stessi nodi e la stessa radice: in B ogni nodo ha come figlio sinistro il primo figlio in T e come figlio destro il fratello successivo in T.

L'equivalenza vale ai fini della previsita. Le sequenze di nodi esaminati su T e B coincidono se T e B sono visitati in previsita.
...
* binpadre: (alberobin, nodo) -> nodo
* figliosinistro: (alberobin, nodo) -> nodo
* figliodestro: (alberobin, nodo) -> nodo
* sinistrovuoto: (alberobin, nodo) -> boolean
* destrovuoto: (alberobin, nodo) -> boolean
* costrbinalbero: (alberobin, alberobin) -> alberobin
* cancsottobinalbero: (alberobin, nodo) -> alberobin
...
* binradice(T) = u
  ...
  * Postcondizione: livello(u) = 0
    * u -> radice di T
* binpadre(T, u) = v
  * Precondizione: T <> (0, 0), u € N, livello(u) > 0
  * Postcondizione: (v, u) € A, livello(v) = livello(u) - 1
    * v -> padre di u
* figliosinistro(T, u) = v
  * Precondizione: T <> (0, 0), u € N, sinistrovuoto(T, u) = false
  * Postcondizione: v -> figlio sinistro di u in T
* figliodestro(T, u) = v
  * Precondizione: T <> (0, 0), u € N, destrovuoto(T, u) = false
  * Postconizione: v -> figlio destro di u in T
* sinistrovuoto(T, u) = b
  * Precondizione: T <> (0, 0), u € N
...
Operatori:
* legginodo: (alberobin, nodo) -> tipoelem
* scrivinodo: (alberobin, nodo, tipoelem) -> alberobin

### Operatori aggiuntivi - Specifica semantica
Tipi:
* tipoelem è del tipo dell'etichetta del nodo (int, char, ...)

Operatori:
* legginodo(T, u) = a
  * Precondizione: T <> (0, 0), u € N, u è un nodo di T
  * Postcondizione: a è il valore associato al nodo n in T
* scrivinodo(T, u, a) = T'
  * Precondizione: T <> (0, 0), n € N, n è un nodo di T
...
Operatori:
* insbinradice: (alberobin, nodo) -> alberobin
* insfigliosinistro: (alberobin, nodo) -> alberobin
* insfigliodestro: (alberobin, nodo) -> alberobin

### Nuovi operatori - Specifica semantica
Operatori:
* insbinradice(T, u) = T'
  * Precondizione: T = (0, 0)
  * Postcondizione: T' = (N, A), N = {u}, livello(u) = 0, A = 0
* insfigliosinistro(T, u) = T'
  * Precondizione: T <> (0, 0), u € N, sinistrovuoto(T, u) = true
  * Postcondizione: N' = N U {v}, T' si ottiene da T aggiungendo v come figlio sinistro di u
* insfigliodestro(T, u) = T'
  * Precondizione: T <> (0, 0), u € N, destrovuoto(T, u) = true
  * Postcondizione: N' = N U {v}, T' si ottiene da T aggiungendo v come figlio destro di u
...
## Grafo
Il grafo è una struttura composta da nodi e archi che rappresenta una relazione binaria sull'insieme costituito dai nodi. I nodi sono usati per rappresentare oggetti e gli archi per rappresentare relazioni tra coppie di oggetti.
...
* insarco: (grafo, nodo, nodo) -> grafo
* cancnodo: (grafo, nodo) -> grafo
* cancarco: (grafo, nodo, nodo) -> grafo
* adiacenti: (grado, nodo) -> lista
* esistenodo: (grafo, nodo) -> boolean
* esistearco: (grafo, nodo, nodo) -> boolean
* legginodo: (grafo, nodo) -> tipoelem
* scrivinodo: (grafo, nodo, tipoelem) -> grafo
...
* crea() = G
  * Precondizione: nessuna
  * Postcondizione: G = (N, A) con N = 0 e A = 0
* vuoto(G) = b
  * Precondizione: nessuna
  * Postcondizione: G = (N, A) con N = 0 e A = 0
    * Vero, b = true
    * Falso, b = false
* insnodo(G, u) = G'
  * Precondizione: G = (N, A), u € N
  * Postcondizione: G' = (N', A) con N' = N U {u}
* insarco(G, u, v) = G'
  * Precondizione: G = (N, A), u € N, v € N, {u, v} !€ A
  * Postcondizione: G' = (N, A') con A' = A U {(u, v)}
* cancnodo(G, u) = G'
  * Precondizione: G = (N, A), u € N
...
* adiacenti(G, u) = L
  * Precondizione: G = (N, A), u € N
  * Postcondizione: L è una lista che contiene una e una sola volta gli elmenti di A(u) = {v € N | (u, v) € A}
* esistenodo(G, u) = b
  * Precondizione: G = (N, A)
  * Postcondizione: u € N
    * Vera, b = true
    * Falsa, b = false
* esistearco(G, u, v) = b
  * Precondizione: G = (N, A), u € N, v € N
  * Postcondizione: (u, v) € A
    * Vera, b = true
    * Falsa, b = false
* legginodo(G, u) = a
  * Precondizione: G = (N, A), u € N
